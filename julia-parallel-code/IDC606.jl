...
# ╔═╡ 2a84f006-07a4-4858-8f9c-d4f71d9cad2e
begin
	using Plots
	using FLoops
...
# ╔═╡ 19d12128-779c-4467-88bf-af768aea2adc
plotly()

# ╔═╡ 77e8c740-8624-4ab8-8d70-e28ae21c573e
Threads.nthreads()

# ╔═╡ dcef7c93-b911-40ca-b7d9-84b06165ae5e
struct Spins
	N::Int32
	M::Int32
	A::Array{Int32}
...
# ╔═╡ a71539d4-3690-49b0-8ec9-4d5db02adf6a
function init(N = 100::Int,M = 100::Int)
	S = Spins(N,M,rand([Int(0),Int(1)], N+2,M+2))
	S.A[1,:] .= 0
	S.A[N + 2, :] .= 0
    S.A[:,1] .= 0 
	S.A[:,M + 2] .= 0
...
# ╔═╡ ee382537-15e4-4d58-b071-4acb6497b87e
html"<br><br><br><br><br>"

# ╔═╡ 6a0aecce-4245-40b6-9fd1-bee93f8af6db
function EnergyParallel(S::Spins)
	E = zeros(Int,S.N+2,S.M+2)
	p = [(-1,0),(1,0),(0,-1),(0,1)]
	@floop ThreadedEx(basesize = 1, nestlevel=Val(:inf), simd = true) for i in p
			Epart = *(S.A[2:S.N+1,2:S.M+1], 
					S.A[2+i[1]:S.N+1+i[1],
						2+i[2]:S.M+1+i[2]])
			@reduce(E += Epart)
	end
...
# ╔═╡ d7b72b9b-e736-4229-84b4-5fab7b2a71dc
function Advance(S::Spins,max_steps=10000::Int)
	
	T = 10
	eps = 0.001

	E = zeros(Int,max_steps + 1)
...
# ╔═╡ 1bda9b22-1a39-453f-ad09-51fbe60e9356
function run_parallel(N=100::Int,M=100::Int,max_steps=10000::Int)	
	S = init(N,M)
	E,m,S=Advance(S,max_steps)
...
# ╔═╡ 865af3bc-748e-4923-bdb5-a0b5af49a680
@time run_parallel(100,100,10000)
...
[[deps.Octavian]]
deps = ["ArrayInterface", "IfElse", "LoopVectorization", "ManualMemory", "PolyesterWeave", "Static", "ThreadingUtilities", "VectorizationBase"]
git-tree-sha1 = "80d81a9ea3e094c13a8c0351eb9d2289b790af67"

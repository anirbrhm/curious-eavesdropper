...
vector<int> E(max_steps + 1, 0) ; 
vector<float> m(max_steps + 1, 0) ;
  ...
  public : 
    ...
    int energy(){
        ...
        for(int i=0 ; i<this->A.size() ; i++){
            for(int j=0 ; j<this->A[0].size()-1 ; j++){
                res += this->A[i][j] * this->A[i][j+1] ; 
            }
        }        

        for(int i=0 ; i<this->A[0].size() ; i++){
            for(int j=0 ; j<this->A.size() ; j++){
                res += this->A[j][i] * this->A[j+1][i] ; 
            }
        }
...
void advance(Spins &S){
    ...
    while ((mod(m[step]) < eps) or ((1 - mod(m[step]))< eps) or (step < max_steps)){
        step += 1 ; 
        int i = rand()%(N) + 1 ; 
        int j = rand()%(M) + 1 ; 
